...
const BASE_DIR = path.join(ROOT, 'project');
...
async function main() {
  ...
  try {
    ...
    console.error('Missing template: project/template.html');
    process.exit(1);
  }
  ...
  await writeSlugPages({ baseDir: BASE_DIR, template, slugs: targets, label: 'project' });
}
